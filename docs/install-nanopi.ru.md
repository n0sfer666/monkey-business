...
Самый простой путь повторяет dev-флоу, но целится в R2S по SSH. Из каталога репозитория:
...
- **Local region** — какой регион считается «локальным» для прямой маршрутизации. Выберите **`Other —
  all traffic direct`**, если хотите режим **passthrough**: туннель настроен, но весь трафик идёт
  напрямую (без VPN-маршрутизации). Кастомные списки и режим в этом случае игнорируются.
