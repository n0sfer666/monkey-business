...
Переходы (только переходы — это бережно к флешке) пишутся в `/usr/local/server.main.log`, а **не** в
`logread`:
...
Чтобы выключить — удалите из `/etc/crontabs/root` строку с **`watchdog.sh`** (строку
`boothealth.sh beat` оставьте: она про бережное обращение с флешкой, а не про VPN).
