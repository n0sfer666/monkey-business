...
|----------|-----|-----------------|
| high | **kill_switch — полноценный тумблер.** Сейчас fail-closed де-факто всегда активен на TPROXY-слое (трафик на мёртвый порт дропается); UCI-опция не переключает поведение и не читается. Tooltip сделан честным. | Настоящий тумблер (вкл/выкл fail-closed, поведение при падении xray) — это работа на firewall-слое + netns-тест (T3). Непроверяемо без отдельной netns-итерации. |
