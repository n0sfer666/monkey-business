...
down_backoff() {
	...
	# Отсчёт от КОНЦА попытки, а не от начала тика: сама попытка имеет право потратить весь
	# DOWN_BUDGET, и первые ступени лестницы (60/120с) на момент возврата уже были бы в прошлом —
	# следующий cron-тик проходил бы гейт сразу. Обещанной паузы не существовало: LAN оставалась
	# под kill-switch почти непрерывно, пока лестница не доберётся до потолка.
	WD_NEXT=$(( $(now) + d ))
}
